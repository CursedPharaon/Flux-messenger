...
export default function Login(){
  ...
  const submit = async(e:React.FormEvent)=>{
    ...
    try{ await login(email,password); nav('/'); } catch(e:any){ setErr(e.response?.data?.error || 'Login failed'); } finally{ setLoading(false); }
  };
    ...
    <div className="min-h-screen bg-[#0a0a0f] flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-[#15151c] border border-white/10 rounded-2xl p-8 shadow-2xl">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-white/50 mt-1">Fast, secure, modern messaging</p>
        </div>
        <form onSubmit={submit} className="space-y-4">
          ...
          <input value={password} onChange={e=> setPassword(e.target.value)} placeholder="Password" type="password" required className="w-full bg-[#1e1e28] border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-violet-600"/>
        ...
        <p className="text-center text-sm text-white/50 mt-6">No account? <Link to="/register" className="text-violet-400 hover:underline">Sign up</Link></p>
      </div>
    </div>
  );
}
